...
void HAL_RCCEx_GetPeriphCLKConfig(RCC_PeriphCLKInitTypeDef  *PeriphClkInit) {
    ...
    PeriphClkInit->PLLI2SDivQ = (uint32_t)((RCC->DCKCFGR1 & RCC_DCKCFGR1_PLLI2SDIVQ)
                                           >> RCC_DCKCFGR1_PLLI2SDIVQ_Pos);
    PeriphClkInit->PLLSAIDivQ = (uint32_t)((RCC->DCKCFGR1 & RCC_DCKCFGR1_PLLSAIDIVQ)
                                           >> RCC_DCKCFGR1_PLLSAIDIVQ_Pos);
    PeriphClkInit->PLLSAIDivR = (uint32_t)((RCC->DCKCFGR1 & RCC_DCKCFGR1_PLLSAIDIVR)
...
HAL_StatusTypeDef HAL_RCCEx_PeriphCLKConfig(RCC_PeriphCLKInitTypeDef
    ...
    if((plli2sused == 1)
        ...
        if(((((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_SAI1) ==
            ...
            assert_param(IS_RCC_PLLI2S_DIVQ_VALUE(PeriphClkInit->PLLI2SDivQ));
            /* Read PLLI2SP and PLLI2SR values from PLLI2SCFGR register (this value is not needed for SAI configuration) */
            tmpreg0 = ((RCC->PLLI2SCFGR & RCC_PLLI2SCFGR_PLLI2SR) >>
                       RCC_PLLI2SCFGR_PLLI2SR_Pos);
            /* Configure the PLLI2S division factors */
            /* PLLI2S_VCO Input  = PLL_SOURCE/PLLM */
            /* PLLI2S_VCO Output = PLLI2S_VCO Input * PLLI2SN */
            /* SAI_CLK(first level) = PLLI2S_VCO Output/PLLI2SQ */
            __HAL_RCC_PLLI2S_CONFIG(PeriphClkInit->PLLI2S.PLLI2SN,
                                    PeriphClkInit->PLLI2S.PLLI2SQ, tmpreg0);
            /* SAI_CLK_x = SAI_CLK(first level)/PLLI2SDIVQ */
            __HAL_RCC_PLLI2S_PLLSAICLKDIVQ_CONFIG(PeriphClkInit->PLLI2SDivQ);
        }
    ...
    if(pllsaiused == 1) {
        ...
        if((((PeriphClkInit->PeriphClockSelection) & RCC_PERIPHCLK_CLK48) ==
            ...
            /* 48CLK = f(PLLSAI clock output) = f(VCO clock) / PLLSAIP */
            __HAL_RCC_PLLSAI_CONFIG(PeriphClkInit->PLLSAI.PLLSAIN,
                                    PeriphClkInit->PLLSAI.PLLSAIP, tmpreg0);
        }
    ...
    if(PeriphClk == RCC_PERIPHCLK_SAI1) {
        ...
        switch(saiclocksource) {
            case 0: { /* PLLSAI is the clock source for SAI1 */
                ...
                frequency = (vcoinput * ((RCC->PLLSAICFGR & RCC_PLLSAICFGR_PLLSAIN) >> 6)) /
                            (tmpreg);
                /* SAI_CLK_x = SAI_CLK(first level)/PLLSAIDIVQ */
            ...
            case RCC_DCKCFGR1_SAI1SEL_0: { /* PLLI2S is the clock source for SAI1 */
                ...
                frequency = (vcoinput * ((RCC->PLLI2SCFGR & RCC_PLLI2SCFGR_PLLI2SN) >> 6)) /
                            (tmpreg);
                /* SAI_CLK_x = SAI_CLK(first level)/PLLI2SDIVQ */
...
HAL_StatusTypeDef HAL_RCCEx_EnablePLLI2S(RCC_PLLI2SInitTypeDef  *PLLI2SInit) {
    ...
    __HAL_RCC_PLLI2S_CONFIG(PLLI2SInit->PLLI2SN, PLLI2SInit->PLLI2SQ,
                            PLLI2SInit->PLLI2SR);
#else
    /* PLLI2S_VCO = f(VCO clock) = f(PLLI2S clock input) * PLLI2SN */
