...
static HAL_StatusTypeDef QSPI_WaitFlagStateUntilTimeout(
    QSPI_HandleTypeDef *hqspi, uint32_t Flag, FlagStatus State, uint32_t Tickstart,
    uint32_t Timeout);
static HAL_StatusTypeDef QSPI_WaitFlagStateUntilTimeout_CPUCycle(
    QSPI_HandleTypeDef *hqspi, uint32_t Flag, FlagStatus State, uint32_t Timeout);
...
HAL_StatusTypeDef HAL_QSPI_Command_IT(QSPI_HandleTypeDef *hqspi,
    ...
    if(hqspi->State == HAL_QSPI_STATE_READY) {
        ...
        hqspi->State = HAL_QSPI_STATE_BUSY;
        /* Wait till BUSY flag reset */
        status = QSPI_WaitFlagStateUntilTimeout_CPUCycle(hqspi, QSPI_FLAG_BUSY, RESET,
                 hqspi->Timeout);
    ...
    if(hqspi->State == HAL_QSPI_STATE_READY) {
        ...
        if(pData != NULL) {
            ...
            while(hqspi->TxXferCount > 0U) {
                ...
                status = QSPI_WaitFlagStateUntilTimeout(hqspi, QSPI_FLAG_FT, SET, tickstart,
                                                        Timeout);
    ...
    if(hqspi->State == HAL_QSPI_STATE_READY) {
        ...
        if(pData != NULL) {
            ...
            while(hqspi->RxXferCount > 0U) {
                ...
                status = QSPI_WaitFlagStateUntilTimeout(hqspi, (QSPI_FLAG_FT | QSPI_FLAG_TC),
    ...
    if(hqspi->State == HAL_QSPI_STATE_READY) {
        ...
        if(pData != NULL) {
            ...
            if(status == HAL_OK) {
                ...
                if(HAL_DMA_Start_IT(hqspi->hdma, (uint32_t)pData,
                                    (uint32_t)&hqspi->Instance->DR, hqspi->TxXferSize) == HAL_OK) {
                    /* Process unlocked */
                    __HAL_UNLOCK(hqspi);
                    /* Enable the QSPI transfer error Interrupt */
    ...
    if(hqspi->State == HAL_QSPI_STATE_READY) {
        ...
        if(pData != NULL) {
            ...
            if(status == HAL_OK) {
                ...
                if(HAL_DMA_Start_IT(hqspi->hdma, (uint32_t)&hqspi->Instance->DR,
                                    (uint32_t)pData, hqspi->RxXferSize) == HAL_OK) {
                    /* Configure QSPI: CCR register with functional as indirect read */
                    MODIFY_REG(hqspi->Instance->CCR, QUADSPI_CCR_FMODE,
                               QSPI_FUNCTIONAL_MODE_INDIRECT_READ);
                    /* Start the transfer by re-writing the address in AR register */
...
HAL_StatusTypeDef HAL_QSPI_AutoPolling(QSPI_HandleTypeDef *hqspi,
                                       QSPI_CommandTypeDef *cmd, QSPI_AutoPollingTypeDef *cfg, uint32_t Timeout) {
    HAL_StatusTypeDef status;
    uint32_t tickstart = HAL_GetTick();
    /* Check the parameters */
    assert_param(IS_QSPI_INSTRUCTION_MODE(cmd->InstructionMode));
...
uint32_t HAL_QSPI_GetFifoThreshold(QSPI_HandleTypeDef *hqspi) {
    return ((READ_BIT(hqspi->Instance->CR,
                      QUADSPI_CR_FTHRES) >> QUADSPI_CR_FTHRES_Pos) + 1U);
}
...
/**
  ...
  */
static HAL_StatusTypeDef QSPI_WaitFlagStateUntilTimeout(
    QSPI_HandleTypeDef *hqspi, uint32_t Flag,
