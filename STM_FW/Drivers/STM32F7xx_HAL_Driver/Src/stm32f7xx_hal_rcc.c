...
uint32_t HAL_RCC_GetPCLK1Freq(void) {
    ...
    return (HAL_RCC_GetHCLKFreq() >> APBPrescTable[(RCC->CFGR & RCC_CFGR_PPRE1) >>
                                         RCC_CFGR_PPRE1_Pos]);
}
...
uint32_t HAL_RCC_GetPCLK2Freq(void) {
    ...
    return (HAL_RCC_GetHCLKFreq() >> APBPrescTable[(RCC->CFGR & RCC_CFGR_PPRE2) >>
                                         RCC_CFGR_PPRE2_Pos]);
}
...
void HAL_RCC_GetOscConfig(RCC_OscInitTypeDef  *RCC_OscInitStruct) {
    ...
    RCC_OscInitStruct->PLL.PLLP = (uint32_t)((((RCC->PLLCFGR & RCC_PLLCFGR_PLLP) +
                                  RCC_PLLCFGR_PLLP_0) << 1) >> RCC_PLLCFGR_PLLP_Pos);
